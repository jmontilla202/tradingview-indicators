...
| `daily-key-levels.pine` | Daily Key Levels | Multi-level session key levels (PDH/PDL, PWH/PWL, Initial Balance, ORB-30, WMH/WML, OVH/OVL, RTH Open) with per-level color, style, and line-width controls. Configurable line style (solid/dashed/dotted) and width. PDH/PDL resets at Globex open (18:00 ET) using the prior RTH range. Labels are plain floating text above each level, positioned at a configurable offset (bars right of current bar, ticks above line) with selectable font size. |
