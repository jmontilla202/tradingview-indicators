...
|`daily-key-levels.pine`|Daily Key Levels|Multi-level session key levels (PDH/PDL, PWH/PWL, Initial Balance, ORB-30, OVH/OVL, RTH Open) drawn as right-extended rays plus moving-average-style continuous curves (RTH VWAP, EMA 8, EMA 21). Static levels share one configurable line style (solid/dashed/dotted) and width; RTH VWAP, EMA 8, and EMA 21 each have their own independent line-style input and are drawn as continuous polylines (not rigid rays) without endpoint labels. PDH/PDL resets at Globex open (18:00 ET) using the prior RTH range; RTH VWAP resets at RTH open (9:30 ET). Static-level labels are plain floating text above each line, positioned at a configurable offset (bars right of current bar, ticks above line) with selectable font size.|
